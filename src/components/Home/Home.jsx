        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {cards.length > 0 ? (
            cards.map((card, index) => (
              <div key={index} className="bg-white p-4 rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300">
                <img
                  src={card.image}
                  alt={card.title}
                ...
                <p className="font-semibold text-xl text-gray-700">Rs- {card.price} /night</p>
              </div>
            ))
          ) : (
            <p>No cards found.</p>
